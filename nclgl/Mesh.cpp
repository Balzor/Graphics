...
void Mesh::Draw() {
	...
	glBindVertexArray(arrayObject);
	if (bufferObject[INDEX_BUFFER]) { //index buffer
...
Vector3 Mesh::GenerateTangent(const Vector3& a,const Vector3& b,const Vector3& c,const Vector2& ta,const Vector2& tb,const Vector2& tc) {
	...
	Vector3 axis = Vector3(vertex1 * coord2.y - vertex2 * coord2.y);
...
void Mesh::GenerateNormals() {
	...
	if (indices) {// generate pervertex normals
		for (GLuint i = 0; i < numIndices; i += 3) {
